...
@app.route("/factory/result/<order_id>")
def factory_result(order_id):
    """Echtes Ergebnis sichtbar machen: liefert das Release-Dokument eines Auftrags als lesbare Seite aus."""
    if not re.fullmatch(r"ord_[\w]+", order_id):
        return "Ungültige Auftrags-ID", 400
    with _factory_lock:
        orders = _load_factory_orders()
    order = next((o for o in orders if o["id"] == order_id), None)
    rel = (order or {}).get("result_file") or ""
    path = (FACTORY_RESULTS_DIR / Path(rel).name) if rel else None
    if not path or not path.exists():
        return "Für diesen Auftrag existiert (noch) kein Release-Dokument — es entsteht erst nach der CEO-Freigabe.", 404
    idea_name = html_module.escape(order.get("idea", {}).get("name", "Auftrag"))
    proto_link = (f'<a href="/factory/prototype/{order_id}" target="_blank" '
                  f'style="color:#38bdf8;">🧪 Produkt (klickbarer Prototyp) im Browser öffnen</a>'
                  if order.get("prototype_file") else "")
    body = html_module.escape(path.read_text(encoding="utf-8"))
    page = (
        '<!DOCTYPE html><html lang="de"><head><meta charset="utf-8">'
        f'<title>📦 Release — {idea_name}</title></head>'
        ...
        f'<h1 style="font-size:1.3rem;">📦 Release-Dokument: {idea_name}</h1>'
        f'<div style="margin:0.6rem 0 1.2rem 0;">{proto_link}</div>'
        f'<pre style="white-space:pre-wrap;background:#1e293b;padding:1.2rem;border-radius:10px;'
        f'font-size:0.85rem;overflow-x:auto;">{body}</pre></div></body></html>')
    return page, 200, {"Content-Type": "text/html; charset=utf-8"}


@app.route("/api/factory/fabi", methods=["POST"])
def factory_fabi_ask():
    """Fabi, der Fabrik-Assistent: beantwortet CEO-Fragen mit Blick auf den Live-Zustand der Fabrik."""
    data = request.get_json(silent=True) or {}
    question = str(data.get("question", "")).strip()[:500]
    if not question:
        return jsonify({"error": "Keine Frage angegeben."}), 400
    if not LLM_ROUTER.any_available():
        return jsonify({"error": "Keine KI-Engine online — siehe Tab 'KI-Gateway'."}), 503

    status_labels = {
        "queued": "wartet auf den CTO", "processing": "in Bearbeitung",
        "awaiting_approval": "wartet auf die CEO-Abnahme", "rejected": "vom CEO abgelehnt",
        "completed": "ausgeliefert", "error": "mit Fehler gestoppt",
    }
    with _factory_lock:
        orders = _load_factory_orders()
    orders.sort(key=lambda o: o.get("created_at", ""), reverse=True)
    lines = []
    for o in orders[:6]:
        step = next((s for s in o.get("steps", []) if s.get("key") == o.get("current_step")), {})
        lines.append(
            f"- „{o.get('idea', {}).get('name', '?')}“ v{o.get('version', 1)}: "
            f"{status_labels.get(o.get('status'), o.get('status'))}"
            f" — aktuelle Station: {step.get('label', '?')}"
            + (f" — Ergebnis liegt unter {o['result_file']}" if o.get("result_file") else ""))
    context = "\n".join(lines) or "- (noch keine Aufträge — die Halle ist leer)"
    try:
        answer, engine = _llm_generate(
            "Du bist FABI, der freundliche LEGO-Fabrik-Assistent im AI-OS Dashboard (Duolingo-Stil: "
            "nett, witzig, motivierend, gern mit Emoji). Du sprichst mit dem CEO. Antworte auf "
            "Deutsch in maximal 3 kurzen Sätzen. Stütze dich ausschließlich auf die Live-Fabrikdaten "
            "und erfinde nichts dazu.",
            f"LIVE-STATUS DER FABRIK (neueste Aufträge zuerst):\n{context}\n\nFRAGE DES CEO: {question}",
            num_predict=260, temperature=0.7, timeout=120)
        return jsonify({"answer": answer.strip(), "engine": engine})
    except Exception as e:
        return jsonify({"error": f"Fabi bekommt gerade keine Antwort vom Modell: {e}"}), 500
